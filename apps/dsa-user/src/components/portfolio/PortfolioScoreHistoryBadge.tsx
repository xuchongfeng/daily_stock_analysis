...
import { PortfolioScoreBadge } from './PortfolioSignalBadges';
...
type ScoreHistoryPoint = { date: string; score: number; advice?: string | null };

type CacheEntry = { points: ScoreHistoryPoint[]; loadedAt: number };
...
export function PortfolioScoreHistoryBadge({
  stockCode,
  score,
  emptyText = '—',
  days = DEFAULT_DAYS,
  ...
  const loadHistory = useCallback(async () => {
    ...
    try {
      ...
      const res = await workbenchHistoryApi.getList({
        stockCode: code,
        startDate,
        page: 1,
      ...
      for (const item of res.items || []) {
        ...
        if (scoreVal == null || !Number.isFinite(Number(scoreVal))) continue;
  ...
  const tooltipContent = useMemo(() => {
    if (loading) {
      return <div className="user-portfolio-tip-muted">正在加载近 {days} 天评分…</div>;
    }
    if (loadError) {
      return <div className="user-portfolio-tip-danger">评分历史加载失败：{loadError}</div>;
    }
    if (points.length === 0) {
      return <div className="user-portfolio-tip-muted">近 {days} 天无评分记录</div>;
    }
    ...
    const muted = '#64748b';
    return (
      <div className="user-portfolio-tip-inner">
        <div className="user-portfolio-tip-header">
          <span>最近 {days} 天评分曲线</span>
          <span className="user-portfolio-tip-latest">最新 {latest}</span>
        </div>
        <div className="user-portfolio-tip-chart">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={chartData} margin={{ top: 6, right: 4, left: -20, bottom: 0 }}>
              <XAxis dataKey="label" tick={{ fontSize: 10 }} stroke={muted} />
              <YAxis domain={[0, 100]} tick={{ fontSize: 10 }} tickCount={5} width={28} stroke={muted} />
              <RechartsTooltip
                contentStyle={{
                  borderRadius: 10,
  ...
  const tip =
    open && typeof document !== 'undefined'
      ? createPortal(
          <span
            role="tooltip"
            className="user-portfolio-score-tip"
            style={{
              top: tipPos.top,
              left: tipPos.left,
              width: tipPos.width,
  ...
  return (
    <>
      <span ref={anchorRef} className={className} onMouseEnter={onEnter} onMouseLeave={onLeave}>
        <PortfolioScoreBadge score={score} emptyText={emptyText} />
      </span>
      {tip}
    </>
  );

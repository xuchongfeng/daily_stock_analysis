...
- [改进] C 端顶栏右上角账号区（头像、用户名）：去掉描边边框，保留浅色底与 hover。
- [改进] C 端登录后顶栏：去掉主导航「账户」项；右上角头像（门户或首字母）与用户名点击进入账户页；取消账号下拉菜单。
- [新功能] ``GET /api/v1/public/demo-analysis-samples``：无需鉴权返回示例个股分析报告（从 ``analysis_history`` 中每代码取最新一条后的近端候选池随机抽样）；认证中间件豁免该路径；C 端「个股分析示例」页优先调用，避免未登录时依赖 ``/api/v1/history`` 导致 Login required。
- [改进] 问股/API 分析：相同标的在未 ``force_refresh`` 时优先复用 ``analysis_history`` 近窗口内最新一条完整记录（不区分门户用户），默认 3 小时，环境变量 ``ANALYSIS_REUSE_TTL_HOURS``（0=关闭）；超时或无记录仍走完整分析。
- [改进] C 端自选页：概念板块标签可跳转「发现 → 板块探索」，链接携带 ``boardName`` 与后台概念映射同名选中对应板块。
- [改进] 持仓快照现价：按市场解析「最近交易日」再取 ``stock_daily`` 收盘价（当日快照用 ``get_effective_trading_date``，历史快照用 ``as_of`` 侧上一交易日）；仅在估值日未早于今天且交易所日历判定当日为交易日时用实时价覆盖，避免休市日异常实时数据；日线查询兼容多种代码写法。
- [修复] 持仓现价：港股日线常以 ``HK00700`` 入库而持仓符号为 ``00700`` 时，``get_latest_close`` 扩展 HK 前缀候选并传入市场参数，避免无法命中日线而退化为均价。
- [改进] C 端「今日」页：页头说明与后台信号摘要对齐并补充个股/板块/概念/AI 叙事等功能说明；历史快照日期在无列表时清空，有持久化快照时默认选中最近一次（列表按日期降序）。
- [改进] C 端营销首页「快速了解」中定价方案摘要：与定价子页一致，改为免费 + ¥19/¥49/¥99 四档及对比维度说明。
- [改进] C 端「个股分析示例」：独立营销路由 `/user/analysis-demo`（与功能介绍等同级顶栏入口），`page-h1` + 功能介绍同款卡片栅格，最多 3 只个股摘要卡片（榜单样本优先，否则近期分析库）；首页「快速了解」保留入口链接；未登录可浏览。
- [改进] ``GET /api/v1/stocks/{code}/quote``：实时行情全部失败时自动回退 ``stock_daily`` 最近交易日收盘价，响应增加可选字段 ``price_source``（``realtime`` / ``daily_close`` / ``placeholder``）；便于区分盘中价与延迟价。
- [改进] 持仓快照持仓行的「现价」在 ``ENABLE_REALTIME_QUOTE`` 开启（默认）且数据源可用时优先使用实时最新价，失败则仍用 ``stock_daily`` 最近收盘价；关闭实时开关时行为与原先一致仅依赖日线。
- [新功能] C 端「发现 · 热点事件」策展模块：`/user/discover/events` 列表与 `/user/discover/events/:slug` 详情（演进时间线、关联板块跳转板块探索、关联个股、核心数据）；后端 `GET /api/v1/discover/hot-events` 与详情接口；数据表 `discover_hot_events` / `discover_hot_event_timeline`，全量导入脚本 `scripts/seed_discover_hot_events.py`（默认读取 `data/discover_hot_events_seed.json`）。
- [新功能] C 端「发现」页增加板块探索入口，`/user/discover/sectors` 对接 `/api/v1/concept-boards`：板块热度（买持/量榜高分/成分数）与成分股评分、建议及标签，与管理后台概念板块一致。
- [改进] C 端持仓「回撤与止损」页：汇总指标改为标签左/数值右对齐并分级着色；止损明细表增加表头底色、行间底色（触发/接近）及现价、亏损比例等列着色。
- [改进] C 端「回测」页：整体表现侧栏标签左/数值右对齐并分级着色（准确率、收益、止损止盈率、胜/负/中性等）；明细表增加表壳与表头层次、hover、按胜负行浅色底及方向/窗口涨跌/结果/状态列着色。
- [新功能] C 端主导航「复盘」改为「回测」，新增 `/user/backtest`（`/user/review` 重定向）：对接 `/api/v1/backtest` 运行与明细，支持按持仓账户筛选组合标的；后端回测 API 增加 `codes`（逗号分隔 / JSON 数组）以聚合组合维度指标与运行多标的回测。
...
- [改进] Web「组合选股」页面改为直接消费后端策略结果，前后端策略口径统一，可复用于后续回测与实盘链路。
- [改进] Web「组合选股」新增策略选择与策略描述区（当前提供策略1），并展示策略回测概览与按入选股票聚合的回测明细（胜率/方向准确率/平均模拟收益）。
- [改进] 组合选股策略1新增硬门槛：仅 `score > 75` 的个股允许进入候选与最终入选流程（含补位），低分个股不再混入12只结果。
- [改进] 组合选股新增“信号日期”可选项：支持按特定日期生成策略榜单；对应回测统计按该信号日期过滤展示，避免与历史全量汇总混淆。

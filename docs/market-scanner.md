...
- **排序规则（核心）**：先按**成交额**全市场降序，再取前 **N**（默认 200）；**成交额相同**时按**成交量**降序。Tushare 为 `daily.amount`（千元）+ `vol`（手）；东财为「成交额」+「成交量」。
...
### 排查：成交量 Top 200「看起来不对」

1. **与自选股去重**：默认 `TOP_MOVERS_DEDUPE_WATCHLIST=true` 会剔除已在 `STOCK_LIST` 中的代码，最终数量可能 **少于 200**。需要完整 200 只时在 `.env` 中设为 `false` 或临时清空自选股。
...
```bash
python main.py --market-scan volume --dry-run --force-run --top-movers-limit 200
# 指定交易日（需为 A 股交易日，且 Tushare 有该日 daily）
python main.py --market-scan volume --dry-run --force-run --top-movers-limit 200 --market-scan-date 2026-04-03
```

...
| 界面 | 双主题工作台 | Web 工作台现支持全新浅色主题与深色主题切换，首页 / 问股 / 回测 / 持仓 / 榜单扫描 / 设置统一升级为同一套视觉与交互体系 |
...
**可选：同花顺概念爬取**（独立任务，不与 `--serve` 同时用）：

```bash
python main.py --crawl ths-concept --dry-run   # 仅解析目录
python main.py --crawl ths-concept             # 全量（需配置环境变量，见 docs/crawler.md）
```
